/**
 * Svelte 组件专用图标数据（静态内联）。
 *
 * 为什么不直接用 `@iconify/svelte`：
 * 它在 SSR 阶段异步加载图标数据，服务端渲染成空占位注释，客户端却渲染出 `<svg>`，
 * 服务端/客户端标记不一致导致 Svelte 5 水合失败（`TypeError: Illegal invocation`），
 * 整个岛（如搜索框）都会失效。这里改为构建期静态内联，两端标记完全一致。
 *
 * 数据来源：
 * - `node_modules/@iconify-json/material-symbols/icons.json`
 * - `node_modules/@iconify-json/fa6-solid/icons.json`
 * 需要新增图标时，从上面的 JSON 或 https://icon-sets.iconify.design/ 复制
 * `body` / `width` / `height` 补进来即可。
 */
