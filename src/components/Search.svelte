...
<div id="search-bar" class="hidden lg:flex transition-all items-center h-11 mr-2 rounded-lg
      ...
      focus-within:outline focus-within:outline-1 focus-within:outline-[var(--primary)]
">
...
<button on:click={togglePanel} aria-label="Search Panel" id="search-switch"
        class="icon-btn h-11 w-11 rounded-lg text-75 hover:text-green lg:!hidden">
  ...
  ">
    ...
    {#each result as item}
       ...
       rounded-xl text-lg px-3 py-2">
            <div class="transition text-90 font-bold group-hover:text-green">
                {item.meta.title}
            </div>
...
<style>
  /* 搜索框的可见焦点圈：细绿一圈，画在**容器**上（focus-within），而不是输入框自身——
     因为桌面端输入框只有左侧 160px 宽，画在它身上会是一小段边框。
     容器同时保留聚焦时背景变深，鼠标/键盘聚焦都能看出焦点在哪（无障碍）。 */
  input:focus {
    outline: 0;
  }
</style>
